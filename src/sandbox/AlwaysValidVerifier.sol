...
///         this is 15 bytes of raw EVM that passes the sandbox opcode scan.
///
///         Bytecode layout (15 bytes):
///
///           ┌─ 8130 Header (7 bytes) ──────────────────────┐
///           │  60 06     PUSH1 0x06   (jump to JUMPDEST)   │
///           │  56        JUMP                               │
///           │  81 30     magic "8130"                       │
///           │  00        version 0                          │
///           │  5B        JUMPDEST                           │
///           └──────────────────────────────────────────────-┘
///           ┌─ Verification Code (8 bytes) ────────────────┐
...
library AlwaysValidSandbox {
    /// @notice Runtime bytecode for the sandbox verifier (15 bytes).
    function bytecode() internal pure returns (bytes memory) {
        return hex"600656813000" // 8130 header (v0)
            hex"5b" //             JUMPDEST
            hex"6001" //           PUSH1 1        (true)
            hex"5f" //             PUSH0          (offset)
            hex"52" //             MSTORE
    ...
    /// @notice Deployment code: 14-byte loader + 15-byte runtime.
