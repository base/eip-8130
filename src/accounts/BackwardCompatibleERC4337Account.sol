...
contract ERC4337Account is Receiver {
        ...
        returns (uint256 validationData)
    {
        ...
        bool valid;
        try ACCOUNT_CONFIGURATION.verify(address(this), userOpHash, abi.decode(userOp.signature, (AccountConfiguration.Verification))) returns (uint8) {
            valid = true;
        } catch {
            valid = false;
        }
    ...
    function isValidSignature(bytes32 hash, bytes calldata signature) external view returns (bytes4) {
        try ACCOUNT_CONFIGURATION.verify(address(this), hash, abi.decode(signature, (AccountConfiguration.Verification))) returns (uint8) {
