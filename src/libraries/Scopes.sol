...
///         actor's config.
///
/// @dev Reference vocabulary, not enforcement. Keystore is deliberately scope-agnostic: it treats
...
///      That is safe against the immutable Keystore deployment because unknown bits grant nothing and
...
library Scopes {
    ...
    /// @notice Operational-actor predicate for account implementations: true for the unrestricted admin
    ///         (`scope == 0`) or a {SENDER} actor that is not gated by a policy ({POLICY} unset). A POLICY-gated
    ///         actor is never operational — it must route every call through its manager — so it fails this test.
    ///
    /// @dev A convenience composed purely from the grant bits; it is not enforcement. Accounts use it to decide who
    ///      may drive execution or sign (ERC-1271): authority an operational actor already holds via direct calls.
    ///
    /// @param scope The actor's stored scope bitmask.
    ///
    /// @return True if `scope` denotes an operational actor.
