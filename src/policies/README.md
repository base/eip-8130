...
gate forces every call that actor makes to land on that single manager. These contracts are one such manager:
one that enforces application-specific limits and then drives the account. `SCOPE_POLICY`
