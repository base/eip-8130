...
///
///      Scope: account-acting {execute} (precompile) and external-caller {executeFor} / {executeForMany}.
contract PolicyManager is ReentrancyGuard {
    ...
    /// @dev External-path validation: manager-match, live commitment vs binding, active actor, then enforce.
    ...
    /// @dev Require the acting actor to still be active (present). A presence check, not a timestamp comparison:
    ///      getActorConfig returns the empty config (zero authenticator) for an expired actor, so an absent
    ///      authenticator here means the actor is no longer active. Required on the external path (no protocol auth);
    ///      omitted on {execute}, where authentication already enforced expiry before dispatch. The commitment match
