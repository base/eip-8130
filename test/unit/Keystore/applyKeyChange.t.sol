...
contract ApplyConfigChangeActorTest is KeystoreTest {
    ...
    /// @notice The zero-sentinel changeType (ChangeType.Invalid) reverts with UnknownChangeType (after the scope-0
    ///         gate). Any out-of-range wire value panics on ABI-decode before reaching this branch, so Invalid is the
    ///         only reachable "unknown" case.
