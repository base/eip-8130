...
contract AuthenticateTest is KeystoreTest {
    ...
    /// @notice A policy-gated actor authenticates and surfaces its scope; its manager resolves via the separate,
    ///         actorId-keyed policy accessor (authentication is orthogonal to policy resolution).
    /// @dev authenticateActor returns scope only; getPolicyManager resolves the stored manager (the commitment is a
    ///      separate execution-time read).
    ...
    /// @notice An ungated actor authenticates and its (separate) policy accessor reports a zero manager.
    /// @dev No manager slot is written for an ungated actor, so getPolicyManager returns address(0).
    ...
    function test_getActorConfig_success_expiredStoredActorReturnsEmpty(uint256 ownerSeed, uint256 actorSeed) public {
        ...
        // Live before expiry.
        Keystore.ActorConfig memory live = keystore.getActorConfig(account, actorId);
        ...
        Keystore.ActorConfig memory dead = keystore.getActorConfig(account, actorId);
    ...
    function test_getActorConfig_success_expiredInlineSelfReturnsEmpty(uint256 ownerSeed) public {
        ...
        uint48 expiry = uint48(block.timestamp + 1 days);
        _authorizeActorWithExpiry(account, ownerPk, selfId, keystore.K1_AUTHENTICATOR(), expiry);

        Keystore.ActorConfig memory live = keystore.getActorConfig(account, selfId);
        assertEq(live.authenticator, keystore.K1_AUTHENTICATOR());
        assertEq(live.expiry, expiry);

        vm.warp(uint256(expiry) + 1);
        Keystore.ActorConfig memory dead = keystore.getActorConfig(account, selfId);
        assertEq(dead.authenticator, address(0));
        assertEq(dead.expiry, uint48(0));
        assertEq(dead.scope, uint16(0));
    }
