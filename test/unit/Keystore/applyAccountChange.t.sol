...
contract AccountLockTest is KeystoreTest {
    ...
    /// @notice applySignedLockChanges reverts UnknownLockOp for LockOp.Invalid (0). Out-of-range ordinals cannot reach
    ///         this path: the enum-typed `op` parameter rejects them at ABI decode.
    function test_applySignedLockChanges_revert_unknownOp(uint256 pkSeed, uint16 delay) public {
        ...
        Keystore.LockOp op = Keystore.LockOp.Invalid;
        uint64 seq = keystore.getChangeSequences(account).local;
    ...
    function test_applySignedActorChanges_revert_whenLocked(uint256 pkSeed, uint16 delay) public {
        ...
        vm.expectRevert(Keystore.AccountIsLocked.selector);
        keystore.applySignedActorChanges(account, chainId, seq, changes, auth);
    }
