...
Two account implementations are deployed: `DefaultAccount` (the bare building block, deployed standalone as the direct EIP-7702 delegation target for EOAs) and `DefaultHighRateSponsoringAccount` (immutable high-rate sponsoring account). `DefaultHighRateSponsoringAccount` inherits from `DefaultAccount` but is deployed as its own singleton, since a smart-account proxy is a permanent address that cannot re-delegate the way a 7702 EOA can. Its limited ETH movement (no outbound value while locked) makes balance predictable to mempools, so ERC-1167 clones of this implementation can be admitted for high-rate sponsoring.
...
| `DefaultHighRateSponsoringAccount` | Deployed account | High-rate sponsoring account (behind a 45-byte ERC-1167 proxy). Blocks outbound ETH when locked so gas fees are the only balance decrease — enabling higher mempool rate limits for sponsored txs |

### Accounts and proxies

Every account is a small per-account proxy (deployed at a deterministic CREATE2 address) that delegatecalls to one shared implementation singleton. The immutable strategy deployed here is an ERC-1167 minimal proxy (45 bytes) in front of `DefaultHighRateSponsoringAccount` — that fixed 1167 delegation is what qualifies the account for high-rate sponsoring. An upgradeable (UUPS) strategy — `UpgradeableAccount` behind `UpgradeableProxy` — is provided as an unaudited example in a separate repository.

`AccountConfiguration.createAccount(userSalt, bytecode, initialActors)` is itself proxy-agnostic — it just `CREATE2`s whatever `bytecode` it's given. The caller decides which proxy strategy to use: an ERC-1167 clone of `DefaultHighRateSponsoringAccount`, or an upgradeable proxy for an upgradeable account.
