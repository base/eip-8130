...
Two account implementations are deployed: `DefaultAccount` (the bare building block, deployed standalone as the direct EIP-7702 delegation target for EOAs) and `CanonicalHighRatePayerAccount` (immutable high-rate self-paying account). `CanonicalHighRatePayerAccount` inherits from `DefaultAccount` but is deployed as its own singleton, since a smart-account proxy is a permanent address that cannot re-delegate the way a 7702 EOA can. While locked, a self-paying high-rate tx cannot move the account's own ETH (outbound value is blocked), so the only balance decrease is gas — making balance predictable to mempools and allowing higher rate limits. ERC-1167 clones of this implementation are the admission path for that high-rate self-paying mode.
...
| `CanonicalHighRatePayerAccount` | Deployed account | High-rate self-paying account (behind a 45-byte ERC-1167 proxy). While locked, cannot move its own ETH in the txn — gas is the only balance decrease — enabling higher mempool rate limits for self-paying txs |

### Accounts and proxies

Every account is a small per-account proxy (deployed at a deterministic CREATE2 address) that delegatecalls to one shared implementation singleton. The immutable strategy deployed here is an ERC-1167 minimal proxy (45 bytes) in front of `CanonicalHighRatePayerAccount` — that fixed 1167 delegation is what qualifies the account for high-rate self-paying. An upgradeable (UUPS) strategy — `UpgradeableAccount` behind `UpgradeableProxy` — is provided as an unaudited example in a separate repository.

`AccountConfiguration.createAccount(userSalt, bytecode, initialActors)` is itself proxy-agnostic — it just `CREATE2`s whatever `bytecode` it's given. The caller decides which proxy strategy to use: an ERC-1167 clone of `CanonicalHighRatePayerAccount`, or an upgradeable proxy for an upgradeable account.
