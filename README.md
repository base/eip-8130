...
Two account implementations are deployed as usable accounts: `DefaultHighRateAccount` (immutable) and `UpgradeableAccount` (general, upgradeable). `DefaultAccount` is the shared building block behind both — its bytecode is embedded in each, so it is not deployed standalone. `Default4337Account` is a separate, opt-in building block for ERC-4337 (see below); nothing deployed by default depends on it.
...
| `UpgradeableAccount` | Deployed account | The general upgradeable account: UUPS-upgradeable `DefaultAccount` (behind `UpgradeableProxy`), with upgrades authorized by a CONFIG-scoped key. Carries no ERC-4337 surface by default — upgrade to a `Default4337Account`-derived implementation if a given deployment needs it |
